...
class SocialDownloadError(Exception):
...
class SocialDownloader:
    ...
    async def inspect(self, url: str) -> SocialInfo:
        ...
        if process.returncode != 0:
            ...
            raise SocialDownloadError(_friendly_error(output))
    ...
    async def download(
        ...
        if return_code != 0:
            ...
            raise SocialDownloadError(_friendly_error(output) or f"O yt-dlp encerrou com codigo {return_code}.")
    ...
    async def _run_gallery_dl(self, url: str) -> list[object]:
        ...
        cmd.extend(self._gallery_common_args())
        cmd.append(url)
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
    ...
    def _gallery_common_args(self) -> list[str]:
        args: list[str] = []
        if self.user_agent:
            args.extend(["--user-agent", self.user_agent])
        if self.cookies_file:
