...
class LitterboxImageHost:
    def __init__(self, endpoint: str = "https://litterbox.catbox.moe/resources/internals/api.php") -> None:
        self.endpoint = endpoint
    ...
    async def upload_bytes(self, filename: str, data_source, content_type: str | None = None) -> str:
        ...
        session = await get_shared_http_session()
        async with session.post(self.endpoint, data=data, timeout=timeout, headers={"User-Agent": "URLUploadBot/2.0"}) as response:
            body = (await response.text()).strip()
            if response.status >= 400:
                raise ImageHostError(f"HTTP {response.status} ao hospedar a imagem.")
        return _validated_url(body, "O Litterbox nao devolveu uma URL valida.")
...
class FallbackImageHost:
    def __init__(self) -> None:
        self.providers = [LitterboxImageHost(), NullPointerImageHost(), CatboxImageHost(), TelegraphUploadHost()]
        ...
        for provider in self.providers:
            for attempt in range(3):
                try:
                    ...
                    failures.append(f"{provider.__class__.__name__}[{attempt + 1}]: {exc}")
